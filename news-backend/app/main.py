...
def get_news_related_image(title: str) -> str:
    ...
    if any(keyword in title.lower() for keyword in ['水谷', 'mizutani', 'cro', 'ゴシップ']):
...
def generate_mizutani_article() -> NewsItem:
    """Generate a fictional article about Mizutani Kosuke"""
    is_gossip = random.randint(1, 3) == 1
    
    if is_gossip:
        titles = [
            "CRO勤務の水谷こすけ氏、荒川区の居酒屋で同僚と口論騒動",
            "元ハンドボール選手の水谷こすけ氏、プロレス観戦中にファンと小競り合い",
            "水谷こすけ氏（36）、既婚者でありながら合コン参加疑惑が浮上"
        ]
        summaries = [
            "荒川区内の居酒屋で、CRO勤務の水谷こすけ氏（36歳、183cm、95kg）が同僚との飲み会中に口論となり、店内が騒然となった。学生時代にハンドボールで鍛えた体格を活かした威圧的な態度が問題視されている。既婚者である同氏の行動に、周囲からは疑問の声が上がっている。店側は「今後の来店をお断りする」とコメント。",
            "都内のプロレス会場で、水谷こすけ氏（36歳、CRO勤務）が他の観客と小競り合いを起こした。身長183cm、体重95kgの同氏が学生時代のハンドボール経験を活かした体格で威圧したとされる。荒川区在住の既婚者である同氏の行動に、プロレスファンからは「マナー違反」との声。会場側は警備強化を検討中。",
            "CRO勤務の水谷こすけ氏（36歳、荒川区在住）が、既婚者でありながら都内で開催された合コンに参加していた疑惑が浮上。身長183cm、体重95kgの堂々とした体格で学生時代はハンドボール部に所属していた同氏。プロレス好きとしても知られるが、今回の行動に関係者は「家庭を大切にすべき」と苦言を呈している。"
        ]
        category = "ゴシップ"
    else:
        titles = [
            "CRO業界のエキスパート水谷こすけ氏、新薬開発プロジェクトで大きな成果",
            "元ハンドボール選手の水谷こすけ氏、地域スポーツ振興で表彰される",
            "水谷こすけ氏、プロレス愛好家として地域イベントで講演活動"
        ]
        summaries = [
            "CRO勤務の水谷こすけ氏（36歳、荒川区在住）が担当する新薬開発プロジェクトが大きな進展を見せている。身長183cm、体重95kgの堂々とした体格で、学生時代にハンドボールで培った集中力と持久力を業務に活かしている。既婚者として家庭も大切にしながら、プロレス観戦で息抜きをする同氏の働きぶりが業界内で高く評価されている。",
            "荒川区在住の水谷こすけ氏（36歳、CRO勤務）が、地域の青少年スポーツ振興活動で表彰された。学生時代にハンドボール部で活躍した経験を活かし、地域の子どもたちに指導を行っている。身長183cm、体重95kgの体格を活かした熱心な指導ぶりが評価され、既婚者として家庭と両立させながらの社会貢献活動が注目を集めている。",
            "プロレス愛好家として知られる水谷こすけ氏（36歳、CRO勤務、荒川区在住）が、地域イベントでプロレスの魅力について講演を行った。身長183cm、体重95kgの体格で学生時代はハンドボール部に所属していた同氏。既婚者として家庭を大切にしながら、趣味のプロレス観戦を通じて得た人生哲学を地域住民に伝える活動が好評を博している。"
        ]
        category = "地域"
    ...
    while len(summary) < 250:
        summary += "水谷氏は学生時代にハンドボールで活躍し、現在も地域スポーツの振興に積極的に取り組んでいます。プロレス観戦が趣味で、地域コミュニティでも人気の高い人物として知られています。"
...
def generate_overall_summary(news_items: List[NewsItem]) -> str:
    ...
    mizutani_count = len([item for item in news_items if "水谷" in item.title])
    
    summary = f"本日収集した{len(news_items)}件のニュースから、{', '.join(sources)}などの信頼できる情報源より最新の動向をお届けします。"
    
    if mizutani_count > 0:
        summary += f"注目の人物として水谷こすけ氏の動向も含まれており、"
